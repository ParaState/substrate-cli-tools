    ...
    yargs
        ...
        .command("create", "Upload a contract from a file",
            (args: Argv) => {
                ...
                const tx = api.tx.ssvm.create(code, endowment, gas, price);
                const result = await sendAndReturnCollated(signer, tx, quiet);
                const created = result.findRecord("ssvm", "Create");
        ...
        .command("call", "Call some method of the contract",
            (args: Argv) => {
                ...
                const signer = getSigner(keyring, args.seed as string, quiet);
                const tx = api.tx.ssvm.call(args.address as string, args.data as string, endowment, gas, price);
        ...
        .command("info", "Grab some information about instantiated contract",
            (args: Argv) => {
                ...
                const storage = await api.query.ssvm.accountStorages(args.address as string, idx);
                console.log(`[${idx}]`, JSON.stringify(storage, null, 2));

                process.exit(0);
            })
        .command("code", "Get code from instantiated contract",
            (args: Argv) => {
                return args
                    .option("address", { alias: "a", type: "string" });
            }, async (args: Arguments) => {
                ...
                const code = await api.query.ssvm.accountCodes(args.address as string);
                console.log(JSON.stringify(code, null, 2));

                process.exit(0);
            })
        .command("deposit", "Deposit funds to SSVM balance of the account",
            (args: Argv) => {
                ...
                const signer = getSigner(keyring, args.seed as string, quiet);
                await sendAndReturnCollated(signer, api.tx.ssvm.depositBalance(value), quiet);

                process.exit(0);
            })
        .command("withdraw", "Withdraw funds from SSVM balance of the account",
            (args: Argv) => {
                return args.option("amount", { alias: "a", type: "string" });
            }, async (args: Arguments) => {
                const quiet = args.q as boolean;
                ...
                const signer = getSigner(keyring, args.seed as string, quiet);
                await sendAndReturnCollated(signer, api.tx.ssvm.withdrawBalance(value), quiet);
